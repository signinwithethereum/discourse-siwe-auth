...
class ValidatorAbiEncodingTest < Minitest::Test
  ...
  def setup
    @encoded = SmartWalletHelpers.encode_validator_args(ADDRESS, HASH, SIGNATURE)
  end

  def test_encoding_matches_viem
    assert_equal EXPECTED_ENCODING, @encoded
  end

  def test_address_left_padded_to_32_bytes
    address_word = @encoded[0, 64]
